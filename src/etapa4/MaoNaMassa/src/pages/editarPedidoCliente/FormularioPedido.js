...
const FormularioPedido = ({
  ...
  const [categorias, setCategorias] = useState(infoPedido.servico_id);
  ...
  const [descricao, setDescricao] = useState(infoPedido.descricao);
  const [cep, setCEP] = useState(infoPedido.localizacao.cep);
  const [rua, setRua] = useState(infoPedido.localizacao.rua);
  const [numero, setNumero] = useState(infoPedido.localizacao.numero);
  const [complemento, setComplemento] = useState(
    infoPedido.localizacao.complemento
  );
  const [bairro, setBairro] = useState(infoPedido.localizacao.bairro);
  const [cidade, setCidade] = useState(infoPedido.localizacao.cidade);
  const [estado, setEstado] = useState(infoPedido.localizacao.estado);
  ...
  const handleEnviarPedido = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://maonamassa-api.onrender.com/contratacao/${infoPedido.id}`,
        {
          method: "PATCH",
          headers: {
  ...
  return (
    <View style={styles.formPedido}>
      ...
      >
        <Text style={styles.btnContainer__text}>Salvar pedido</Text>
      </TouchableOpacity>
    </View>
  );
};
