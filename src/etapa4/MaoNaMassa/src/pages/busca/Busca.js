...
const Busca = ({ navigation }) => {
  ...
  useEffect(() => {
    if (inputRef.current) {
      ...
      inputRef.current.setSelection &&
        inputRef.current.setSelection({ start: 0, end: termo.length });
    }
  }, [inputRef]);
  ...
  const resultados = useMemo(() => {
    ...
    return (PROFISSIONAIS || []).filter((p) => {
      ...
      const ufOk =
        !uf ||
        String(end0?.estado || "").toUpperCase() === String(uf).toUpperCase();
  ...
  const aplicarFiltros = useCallback(() => {
    if (
      (termo || categoriasSel.length || cidade || uf) &&
      resultados.length === 0
    ) {
      Toast.error(
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.resultHeader}>
        <Text style={styles.resultText}>
          {resultados.length}{" "}
          {resultados.length === 1 ? "profissional" : "profissionais"}{" "}
          encontrado
