...
const CardAvaliacao = ({ pedido }) => {
  ...
  useEffect(() => {
    async function fetchCliente() {
      // Requisição com query param do id do avaliador
      if (!pedido?.avaliador_id) return;
      try {
        const response = await fetch(
          `https://maonamassa-api.onrender.com/users?id=${pedido.avaliador_id}`
        );
  ...
  return (
    <View style={styles.containerAvaliacao}>
      <View style={styles.containerAvaliacao__title}>
        <Avatar.Image size={32} source={{ uri: cliente.foto_blob }} />
        <View>
