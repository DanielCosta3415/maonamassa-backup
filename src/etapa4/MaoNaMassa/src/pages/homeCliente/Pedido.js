...
const Pedido = ({ infoPedido, user }) => {
    ...
    async function fetchProfissional() {
      try {
        ...
        const profResp = await fetch(
          `https://maonamassa-api.onrender.com/professional?id=${infoPedido.professional_id}`
        );
        if (!profResp.ok)
          throw new Error("Erro ao buscar dados em /professional");
  ...
  return (
    <View>
      <TouchableOpacity onPress={handleDetalhePedido}>
