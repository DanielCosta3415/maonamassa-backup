...
function statusToTab(status) {
  ...
  if (s.includes("novo")) return "novos";
  if (s.includes("aceit")) return "aceitos";
  if (s.includes("andam")) return "andamento";
  if (s.includes("conclu")) return "concluidos";
  if (s.includes("cancel")) return "cancelados";
  return "novos";
}

function normalizarPedido(raw, index) {
  const id = raw?.id ?? index + 1;
  const number = raw?.numero ?? raw?.number ?? `#${id}`;
...
async function logout(navigation, atualizarLogin) {
  try {
...
const HomeProfissional = ({ atualizarLogin }) => {
  ...
  useEffect(() => {
    async function carregar() {
      try {
        setCarregando(true);
        setErro("");
        const res = await fetch(`${API_BASE_URL}/contratacao`);
        if (!res.ok) {
        ...
        const normalizados = lista.map((item, index) =>
          normalizarPedido(item, index)
        );
        setOrders(normalizados.length > 0 ? normalizados : MOCK_ORDERS);
      } catch (e) {
    ...
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <View style={styles.header}>
        ...
        <View style={styles.buttonsRow}>
          <Pressable hitSlop={10} style={styles.button}>
          ...
          <Pressable
            hitSlop={10}
            style={styles.button}
            onPress={() => logout(navigation, atualizarLogin)}
          >
      ...
      <FlatList
        ...
        renderItem={({ item }) => <OrderCard navigation={navigation} order={item} />}
  ...
  return (
    <View style={styles.card}>
      ...
      <Pressable
        ...
        onPress={() => navigation.navigate("AtualizarPedido", { order })}
      >
...
const styles = StyleSheet.create({
  ...
  button: { backgroundColor: "#FFFFFFAA", padding: 8, borderRadius: 14 },
  searchWrap: {
    marginHorizontal: 20,
    marginBottom: 16,
    height: 44,
